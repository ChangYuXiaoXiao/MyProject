...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    if ([defaults objectForKey:@"accountName"]!=nil) {
        self.delegate = (id<AppDelegateDelegate>)self.window.rootViewController;
        [self.delegate socketConnecting];
        NSString *userName = [defaults objectForKey:@"userName"];
        NSString *passWord = [defaults objectForKey:@"passWord"];
        NSString *sentStr = [NSString stringWithFormat:@"MsgId=0&Sid=&Uid=0&ActionID=%d&Pid=%@&Pwd=%@",[CYLActionID logIn],userName,passWord];
...
{
    [self.delegate socketDisConnected:err];
...
- (NSURL *)applicationDocumentsDirectory {
    // The directory the application uses to store the Core Data store file. This code uses a directory named "ustc.____" in the application's documents directory.
